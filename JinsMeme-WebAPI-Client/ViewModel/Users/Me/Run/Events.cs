...
using static JinsMeme_WebAPI_Client.API.Users.Me.Run.Events;

namespace JinsMeme_WebAPI_Client.ViewModel.Users.Me.Run
{
...
#pragma warning restore 0067
        ...
        public ReactiveProperty<string> Type { get; } = new ReactiveProperty<string>("computed_data2"); // fixed
        ...
        public IEnumerable<Fields> Fields
        {
            get
            {
                if (Fields_id.Value || Fields_updated_at.Value || Fields_json_data.Value || Fields_data_file_url.Value || Fields_data_created_at.Value)
                {
                    ...
                    if (Fields_id.Value)
                    {
                        fields.Add(API.Users.Me.Run.Events.Fields.id);
                    }
                    if (Fields_updated_at.Value)
                    {
                        fields.Add(API.Users.Me.Run.Events.Fields.updated_at);
                    }
                    if (Fields_json_data.Value)
                    {
                        fields.Add(API.Users.Me.Run.Events.Fields.json_data);
                    }
                    if (Fields_data_file_url.Value)
                    {
                        fields.Add(API.Users.Me.Run.Events.Fields.data_file_url);
                    }
                    if (Fields_data_created_at.Value)
                    {
                        fields.Add(API.Users.Me.Run.Events.Fields.created_at);
                    }
                    return fields;
                }
                return null;
